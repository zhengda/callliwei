...
case class CategoryX(name: String, amount: Int = 0, indexies: List[String])
...
class Application @Inject()(db: Database, dbapi: DBApi, cache: CacheApi, cached: play.api.cache.Cached)
        ...
        db.withConnection {
          implicit conn =>
            ...
            if (category != null) {
              ...
              group match {
                case "district" =>
                  SQL("select * from district2name where name={name}")
                    .on("name" -> category).as(cparser.singleOpt) match {
                    case None => NotFound
                    case Some(cat: Category) =>
                      ...
                      Ok(views.html.list(result.map { x => repack(x) }, group, category))
                  }
                case "profession" =>
                  SQL("select * from profession2name where name={name}")
                    .on("name" -> category).as(cparser.singleOpt) match {
                    case None => NotFound
                    case Some(cat: Category) =>
                      val ids: List[String] = cat.indexies.split(' ').toList
                ...
                case "party" =>
                  SQL("select * from party2name where name={name}")
                    .on("name" -> category).as(cparser.singleOpt) match {
                    case None => NotFound
                    case Some(cat: Category) =>
                      val ids: List[String] = cat.indexies.split(' ').toList
              ...
              q match {
                case Some(value) =>
                  ...
                  Ok(views.html.list(result.map { x => repack(x) }, null, null))
                case None =>
                  val result: List[Liwei] = SQL"select * from t9a ".as(parser.*)
                  Ok(views.html.list(result.map { x => repack(x) }, null, "all"))
                //case _ => NotFound
              }
            }
        }
  ...
  def repack(x: Category) = {
    CategoryX(x.name, x.amount, x.indexies.split(" ").toList)
  }
  ...
  def profile(name: String) = cached.status(_ => "profile200." + name, 200, 24 * 60 * 60).includeStatus(404, 24 * 60 * 60) {
    Action {
      implicit request =>
        import anorm.{Macro, RowParser}
        db.withConnection {
          implicit conn =>
            ...
            val x: LiweiX = cache.getOrElse[LiweiX]("profile" + name) {
              val x = SQL("select * from t9a where name={name} or ename={ename}")
                .on("name" -> name, "ename" -> name).as(parser.singleOpt)
              x match {
                case None => null
                case Some(x) => repack(x)
              }
  ...
  def apiprofile(name: String) = cached.status(_ => "apiprofile200." + name, 200, 24 * 60 * 60).includeStatus(404, 24 * 60 * 60) {
    Action {
      implicit request =>
        import anorm.{Macro, RowParser}
        db.withConnection {
          implicit conn =>
            ...
            //            implicit val writes = Json.writes[LiweiX]
            //            implicit val writes: Writes[LiweiX] = (
            //              (JsPath \ "name").write[String] and
            //                (JsPath \ "ename").write[String]
            //              ) (unlift(LiweiX.unapply))
            //implicit val liweiXWrites = Json.writes[LiweiX]
            //implicit val liweiXFmt = Json.format[LiweiX]

            if (x != null) {
              import ai.x.play.json.Jsonx
              implicit val fmt = Jsonx.formatCaseClass[LiweiX]
              Ok(Json.toJson(x))
            } else {
              NotFound
            }
  ...
  def apicategory(group: String) = cached.status(_ => "apicategory200." + group, 200, 24 * 60 * 60).includeStatus(404, 24 * 60 * 60) {
    Action {
      implicit request =>
        import anorm.{Macro, RowParser}
        db.withConnection {
          implicit conn =>
            val parser: RowParser[Category] = Macro.indexedParser[Category]
            import ai.x.play.json.Jsonx
            implicit val fmt = Jsonx.formatCaseClass[CategoryX]
            group match {
              case "district" =>
                val result: List[Category] = SQL("select * from district2name ").as(parser.*)
                Ok(Json.toJson(result.map { x => repack(x) }))
              case "profession" =>
                val result: List[Category] = SQL("select * from profession2name ").as(parser.*)
                Ok(Json.toJson(result.map { x => repack(x) }))
              case "party" =>
  ...
  def apilist(q: Option[String], group: String, category: String) = cached.status(_ => "apilist200." + q + "." + group + "." + category, 200, 24 * 60 * 60) {
    Action {
      implicit request =>
        import anorm.{Macro, RowParser}
        db.withConnection {
